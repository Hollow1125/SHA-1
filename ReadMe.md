...
- В разделе **Компоновщик -> Общие -> Дополнительные каталоги библиотек** добавьте путь `.../cryptopp/x64/Output/Debug` (или `.../cryptopp/x64/Output/Release`, смотря как библиотека была собрана)
...
Для запуска программы откройте командную строку и перейдите в каталог с исполняемым файлом: `...\SHA1\x64\Debug\`.
...
После запуска програмы для подсчета хэш-суммы файла с расширением `.rar` и размером 13.4 ГБ были получены следующие результаты:
